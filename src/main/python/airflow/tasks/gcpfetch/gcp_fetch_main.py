...
if __name__ == "__main__":
    ...
    practitioners_bucket = secret_manager.access_secret_version(
        "GCP_PRACTITIONERS_GUIDE_SITES_BUCKET"
    )
    other_articles_bucket = secret_manager.access_secret_version(
        "GCP_OTHER_ARTICLES_BUCKET"
    )
    ...
    practitioners_guides_json = fetcher.get_practitioner_guide_md(
        bucket_name=practitioners_bucket
    )
    ...
    all_assets = GCPFetcher.concatenate_unique_ordered(
        parsed_sitemaps_list, practitioners_list, other_articles_list
    )

    output_file_name = "all_assets.txt"

    fetcher.write_to_gcs(
        data=all_assets, bucket_name=processed_docs_bucket, file_name=output_file_name
    )
